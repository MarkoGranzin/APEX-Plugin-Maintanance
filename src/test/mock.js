...
- HEADERS/LABELS MUST BE FULLY VISIBLE — NOT CLIPPED OR OVERLAPPED: column headers, section/group headers and titles must show completely. Watch out for two common bugs: (a) a sticky/absolute group or section header that sits ON TOP of the column-title row and hides it — give the column titles their own reserved space (or correct z-index/order) so a sticky row never covers them; (b) the scroll/overflow container clipping the first row of titles at the top — reserve enough top padding/height so nothing is cut off. Give #mock-root enough height (it is fine to make the page tall and let it scroll) so the WHOLE plugin is visible. After rendering, the column/section headers must read fully (e.g. "To Do", "In Progress", "Done" — not half-cut).
